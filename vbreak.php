...
$sql1= "SELECT * from breakr";
        ...
        if ($numrows!=0) {
         ...
         while ($row =mysqli_fetch_assoc($result)) {
                ...
                $_SESSION['tid']=$row["eid"];
              
                    echo' <form method="post" action="dbreak.php">';
                    echo' <input type="hidden" name="name" value="'.$row["eid"].'" />';
                    echo' <input type="hidden" name="name1" value="'.$row["bid"].'" />';
                    echo'   <td><button class="btn btn-lg " >Approve</button></td>';
                    echo'</form>';
