...
<body style="overflow-x:hidden">
    ...
    if ($numrows!=0) {
        while ($row =mysqli_fetch_assoc($result)) {
            ...
            if (($e==$e1)||($e==$e2)) {
                ...
                $_SESSION['varname'] = $e1;
                echo' <div>';
                echo'   <form name="emp" action="w4.php " onsubmit="return validate_form();" method="post">';
                ...
                echo'   <tr>';
                echo'        <td colspan="3" align="center" id="abc"><b>EMPLOYEE PROFILE.</b></td>';
                ...
                echo' </tr>';
                echo'       <tr>';
                echo'           <td align="left" valign="top" width="41%">Employee Name<span></span></td>';
                
                echo'         <td width="57%">'.ucfirst($row["ename"]).'</td>';
                echo'       <td width="57%"><input type="text" placeholder="'.ucfirst($row["ename"]).'" size=30 name="m1" required></td>';
                echo'     <tr>';
              ...
              //  echo'       <input type="hidden" name="eid1" value="'.$e1.'">';
                ...
                echo'           <td width="57%">' .$row["eemail"].'</td>';
                echo'       <td width="57%"><input type="email" size=30 placeholder="'.$row["eemail"].'" name="m11" required></td>';

                echo'        </tr>';
                echo'      <tr>';
             
                echo'        <td align="left" valign="top" width="41%">Contact Number</td>';
                echo'       <td width="57%">' .$row["ephone"].'</td>';
                echo'       <td width="57%"><input type="number" onkeypress="return isNumberKey(event); size=30 placeholder="'.$row["ephone"].'" name="m3" required></td>';
                echo'    </tr>';
                echo'     <tr>';
                echo'          <td align="left" valign="top" width="41%">Address</td>';
                echo'   <td width="60%" >' .ucfirst($row["eaddr"]).'</td>';
                echo'       <td width="57%"><input type="text" size=30 placeholder="'.ucfirst($row["eaddr"]).'" name="m2" required></td>';
                echo'      </tr>';
                
                echo'    <tr>';
                echo'           <td align="left" valign="top" width="41%">Employee Salary<span></span></td>';
                echo'            <td width="57%">' .ucfirst('Rs.'.$row["esalary"]).'</td>';
                echo'       <td width="57%"><input type="number" size=30 placeholder="'.ucfirst('Rs.'.$row["esalary"]).'" name="m4" required></td>';
                echo'        </tr>';
                echo'       <tr>';
                echo'           <td align="left" valign="top" width="41%">Employee Gender<span></span></td>';
                echo'           <td width="57%">' .ucfirst($row["egender"]).'</td>';
                echo'       <td width="57%"><input type="text" size=30 placeholder="'.ucfirst($row["egender"]).'" name="m5" required></td>';
                echo'      </tr>';
                echo'      <tr>';
                echo'          <td align="left" valign="top" width="41%">Qualification</td>';
                echo'          <td width="57%">' .ucfirst($row["equali"]).'</td>';
                echo'       <td width="57%"><input type="text" size=30 placeholder="'.ucfirst($row["equali"]).'" name="m6" required></td>';
                echo'    </tr>';
                echo'   <tr>';
                echo'        <td align="left" valign="top" width="41%">Skills</td>';
                echo'         <td width="57%" >' .ucfirst($row["eskills"]).'</td>';
                echo'       <td width="57%"><input type="text" size=30 placeholder="' .ucfirst($row["eskills"]).'" name="m7" required></td>';
                echo'      </tr>';
                echo'      <tr>';
                echo'          <td align="left" valign="top" width="41%">Job Location</td>';
                echo'          <td width="57%">'.ucfirst($row["ejobloc"]).'</td>' ;
                echo'       <td width="57%"><input type="text" size=30 placeholder="'.ucfirst($row["ejobloc"]).'" name="m8" required></td>';

                echo'    </tr>';

                echo'       <tr>';
                echo'          <td align="left" valign="top" width="41%">Designation</td>';
                echo'          <td width="57%">'.ucfirst($row["edesig"]).'</td>';
                echo'       <td width="57%"><input type="text" size=30 placeholder="'.ucfirst($row["edesig"]).'" name="m9" required></td>';
                echo'     </tr>';
                echo'     <tr>';
                echo'         <td align="left" valign="top" width="41%">Domain</td>';
                echo'          <td width="57%">'.ucfirst($row["edomain"]).'</td>';
                echo'       <td width="57%"><input type="text" size=30 placeholder="'.ucfirst($row["edomain"]).'" name="m10" required></td>';
                echo'       </tr>';
  ...
  echo'   function validate_form() {';
    echo'       if (document.emp.emp_name.value == "") {';
        echo'          alert("Please fill in the \'Your Employee Name\' box.");';
                echo'            return false;';
                echo'      }';
                echo'     if (document.emp.num.value == "") {';
                    echo'        alert("Enter Employee Number");';
              echo' return false;';
                echo'    }';
                ...
                echo'    if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/).test(document.emp.eem.value)) {';
                ...
                echo'   function isNumberKey(evt) {';
                    echo'    var f=0;';
                    echo'   var p=document.forms["emp"]["m3"].value;';
                    echo'   if(p.length>=10)';
                    echo'   {';
                        echo'       alert("Phone Number Limit Is 10 Digits");';
                        echo'       return false;';
                        echo'    }';
            
                                       echo'  var charCode = (evt.which) ? evt.which : event.keyCode;';
                                       echo'  if (charCode != 46 && charCode > 31 &&';
                                       echo'      (charCode < 48 || charCode > 57)) {';
                    echo'        alert("Enter Number");';
                    echo'      return false;';
                    echo'   }';
                    echo'    return true;';
                    echo'  }';
                    echo'  function isNumberKey1(evt) {';
                        echo'    var charCode = (evt.which) ? evt.which : event.keyCode;';
                        echo'    if ((charCode > 48 && charCode < 57 ||charcode >96 && charcode <105)) {';
                            echo'        alert("Enter Only Character");';
                            echo'       return false;';
                            echo'    }';
                            echo'   return true;';
                            echo'   }';
